...
from bnz.utils cimport free_2d_array, free_3d_array, print_root
from coord_cy cimport init_coord, free_coord_data
...
cdef class GridData:
  ...
  def __cinit__(self, GridCoord *gc):
    ...
    IF MFIELD:
      ...
      self.bf = np.zeros(sh_3, dtype=np_real)
    ELSE:
...
cdef void init_params(GridCoord *gc, bytes usr_dir):
...
IF MPI:
  ...
  cdef void domain_decomp(GridCoord *gc, bytes usr_dir):
