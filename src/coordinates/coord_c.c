...
void lapl_perp1(real *ajm1, real *aj0, real *ajp1, real *ajp2,
...
#pragma omp simd
  for (int i=0; i<nx; ++i) {
    ...
    tmp1[i] = h1 * (am1 + ap1) + h0 * a0;
    ajm1[i] = tmp2[i];

    tmp2[i] = h1 * (a0  + ap2) + h0 * ap1;
    aj0[i] = tmp1[i];
...
void lapl_perp2(real *ajm1, real *aj0, real *ajp1, real c, int nx) {
...
#pragma omp simd
  for (int i=0; i<nx; ++i)
    aj0[i] = h1*(ajm1[i] + ajp1[i]) + h0*aj0[i];

}

void copy1d(real *a, real *b, int n) {
