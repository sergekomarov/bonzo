...
from src.coord cimport lind2gcrd, lind2gcrd_x,lind2gcrd_y,lind2gcrd_z
  ...
  for k in range(gp.k1, gp.k2+1):
    for j in range(gp.j1, gp.j2+1):
      for i in range(gp.i1, gp.i2+1):
        ...
        B[0,k,j,i] =  (Az[j+1,i] - Az[j,i]) / gp.dl[1] / sqrt(phys.beta) + 1e-20
        B[1,k,j,i] = -(Az[j,i+1] - Az[j,i]) / gp.dl[0] / sqrt(phys.beta) + 1e-20
...
cdef double hst_var1(BnzSim sim):
  ...
  return B2 / (2*gp.Nact_glob[0]*gp.Nact_glob[1]*gp.Nact_glob[2])
